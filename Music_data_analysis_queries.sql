...
select artist.name , count(artist.artist_id) as songs_count from artist
...
WITH most_popular_genre AS (
	...
	ROW_NUMBER() OVER(PARTITION BY customer.country ORDER BY COUNT(invoice_line.quantity) DESC) as RW
	FROM invoice_line
...
SELECT * FROM most_popular_genre WHERE RW=1;
